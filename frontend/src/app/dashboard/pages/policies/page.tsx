...
export default function PolicyPagesEditor() {
  const [selected, setSelected] = useState(POLICY_PAGES[0].slug);
  const page = POLICY_PAGES.find((p) => p.slug === selected)!;

  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-serif text-white">Policy Pages</h1>
        <p className="text-sm text-gray-500 mt-1">Write or paste full policy content using Markdown. Automatically rendered as HTML.</p>
      </div>
